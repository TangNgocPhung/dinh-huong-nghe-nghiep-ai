...
(function () {
  ...
  const revealTargets = document.querySelectorAll(".card, .career-card, .about-panel, .pillar, .quiz-question, .overview-copy, .overview-tool, .overview-note, .theory-copy, .riasec-type, .mi-type, .mbti-axis, .mbti-code-pill, .disc-type, .motive-type, .guide-step, .riasec-guide-chip, .onet-embed-section, .external-assessment, section > h2");
  ...
  if (progressBox) {
    ...
    const completed = ids.filter((id) => localStorage.getItem(`dhnn_result_${id}`)).length;
